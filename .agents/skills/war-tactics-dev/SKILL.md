---
name: war-tactics-dev
description: "Operational development skill for War Perang Tactics — Godot 4.7+ tactical turn-based strategy game. Covers architecture invariants, offline asset derivation pipeline, 38px sprite normalization, map generation, building claims, combat formulas, and strict anti-patterns."
---

# ⚔️ War Tactics Developer Skill (`war-tactics-dev`)

This skill defines the technical workflows, coding standards, architectural invariants, and asset pipelines for **War Perang Tactics** (Godot 4.7+ GL Compatibility).
...
> [!CAUTION]
> **FORBIDDEN**: Never use `get_node("/root/...")` to communicate between systems. Never manipulate sibling nodes directly.
...
> [!IMPORTANT]
> **No Runtime Palette Shader**: Do NOT use `needs_palette_tint` or `faction_tint.gdshader`. Faction colors and accessories are baked directly into the derived textures to preserve 2D draw call batching and deliver distinct role silhouettes.
...
Before concluding any task or committing changes:
```bash
# 1. Run full headless test suite
godot --headless --path . scenes/test_battlefield.tscn
godot --headless --path . scenes/test_upgrade_flow.tscn
godot --headless --path . scenes/test_village_capacity.tscn
godot --headless --path . scenes/test_combat_mechanics.tscn
godot --headless --path . scenes/test_popup_and_map.tscn
godot --headless --path . scenes/test_all_units.tscn
godot --headless --path . scenes/TestGridScene.tscn --quit-after 50

# 2. Update Documentation & Graph
# Update CHANGELOG.md, docs/Roadmap.md, and run:
graphify update
```
