...
1. **MCP Engine Testing (`godot-ai`)**:
   - Run the full suite via `test_run(verbose=True)` on `godot-ai` MCP server (`battlefield`, `combat`, `economy`, `units` in `tests/`).
   - Run the headless engine check:
     ```bash
